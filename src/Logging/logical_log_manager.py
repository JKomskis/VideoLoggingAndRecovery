...
class LogicalLogManager():
    ...
    def log_logical_update_record(self, txn_id: int, file_url: str, update_arguments: ObjectUpdateArguments) -> int:
        LoggingManager().log(f'Update, txn {txn_id} name {file_url} using {update_arguments}', LoggingLevel.DEBUG)
        return self._write_log_record(LogRecordType.LOGICAL_UPDATE, txn_id, [
            file_url.encode("utf8"), update_arguments.serialize()
        ])
        # write log record that includes txn id, url of video updated, and update operation
