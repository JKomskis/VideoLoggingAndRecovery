...
class LoggingProfilePartitioned(AbstractProfile):
    def __init__(self, hybrid_protocol, storage_engine, dataframe_metadata):
        super().__init__(profile_name=('logging_hybrid' if hybrid_protocol else 'logging_logical'))
    ...
    def _setUp(self):
        ...
        self.update_operations = [ObjectUpdateArguments('invert_color', 0, 4499) for i in range(NUM_UPDATES)]
...
if __name__ == '__main__':
    ...
    # Logical logging
    profile = LoggingProfilePartitioned(False, storage_engine, dataframe_metadata)
    profile.run_profile()

    # Hybrid logging
    profile = LoggingProfilePartitioned(True, storage_engine, dataframe_metadata)
    profile.run_profile()
